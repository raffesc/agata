...
$frontend_inseriti = array();


foreach ($data->attributes as &$elem) {
    
    $frontend = new BackEndAttribute();
    $frontend->setDB($db);

    $frontend->id_back_end = $elem->id_back_end;
    $frontend->title = $elem->title;
    $frontend->value = $elem->value;
    $frontend->priv = $elem->priv;
    $frontend->type = $elem->type;
    if (!empty($frontend->id_back_end) and !empty($frontend->title) and !empty($frontend->value) and  !empty($frontend->priv) and !empty($frontend->type)) {
        ...
        $frontend->id = $max_id;
        if ($frontend->insert()) {
            array_push($frontend_inseriti, $frontend);
        } 
...
// show products data in json format
echo json_encode(array("records" => $frontend_inseriti));
