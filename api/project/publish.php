...
if (empty($project->id_owner) || empty($project->id)) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        if ($id == $project->id and $id_owner == $project->id_owner) {
            ...
            if ($project->publish()) {
                ...
                // show products data in json format
                echo json_encode(array("response" => "Progetto pubblicato correttamente! Vai nella sezione esplora."));
                exit();
            }
...
http_response_code(404);
// show products data in json format
echo json_encode(array("errore" => "Errore durante la pubblicazione del progetto. Riprovare"));
